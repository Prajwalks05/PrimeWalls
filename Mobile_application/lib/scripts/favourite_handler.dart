...
class FirebaseHandler {
  ...
  static Future<bool> addFavourite(String imageUrl) async {
    try {
      ...
      final sanitizedEmail = sanitizeEmail(email); // Sanitize email before using it
      final docRef = FirebaseFirestore.instance
          .collection('favourites')
          .doc(sanitizedEmail); // Use sanitized email as Firestore document ID

      await docRef.set({
        'fav': FieldValue.arrayUnion([imageUrl]) // Add image URL to the fav array
  ...
  static Future<bool> removeFavourite(String imageUrl) async {
    try {
      ...
      await docRef.set({
        'fav': FieldValue.arrayRemove([imageUrl]) // Remove image URL from the fav array
      }, SetOptions(merge: true)); // Merge to avoid overwriting other data
  ...
  static Future<List<String>> getFavourites() async {
    try {
      ...
      if (snapshot.exists && snapshot.data() != null) {
        ...
        if (data['fav'] is List) {
          return List<String>.from(data['fav']); // Return the list of image URLs
        }
      }
    } catch (e) {
  ...
  static Future<bool> isFavourite(String imageUrl) async {
    ...
    return favourites.contains(imageUrl); // Check if the image URL is in the list of favourites
  }
  ...
  static Future<bool?> toggleFavourite(String imageUrl) async {
    try {
      ...
      if (isFav) {
        final removed = await removeFavourite(imageUrl); // Remove if already favourite
